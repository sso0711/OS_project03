...
// 3level page table을 돌며 ppn + offset으로 물리 주소 구함
pte_t *
...
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for (i = 0; i < sz; i += PGSIZE) // 부모의 가상 주소 0부터 sz까지 반복 복사
  {
    if ((pte = walk(old, i, 0)) == 0) // 가상주소 i에 해당하는 PTE를 부모 페이지테이블에서 찾기
      panic("uvmcopy: pte should exist");
    if ((*pte & PTE_V) == 0) // valid 비트가 0이면 pte에 매핑된 물리 메모리가 없는 것
      panic("uvmcopy: page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte); // 부모 PTE의 flags 추출

    *pte = *pte & ~PTE_W;        // 처음에는 read-only로 설정
    *pte = *pte | PTE_RSW;       // COW bit 설정
    new_flags = PTE_FLAGS(*pte); // 자식 PTE의 수정된 flags

    // 자식의 pagetable new에 가상 주소 i와 부모의 물리 페이지 pa를 매핑
